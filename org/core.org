#+title: Naviserver on AltLinux image using S6.
...
:CUSTOM_ID: about
:END:
This is [[https://wiki.tcl-lang.org/page/NaviServer][NaviServer]] on [[https://hub.docker.com/_/alt][ALT Linux base docker image]] (version sisyphus) using [[https://github.com/just-containers/s6-overlay][s6-overlay]].  To install the NaviServer I used code from  [[https://github.com/gustafn/install-ns][Gustaf Neumann]] script [[https://github.com/gustafn/install-ns/blob/master/install-oacs.sh][install-ns.sh]].   The base image is [[https://hub.docker.com/r/oupfiz5/alt-s6-naviserver][oupfiz5/alt-s6-naviserver]].
...
Build image using multi steps  (by means of docker from [[https://hub.docker.com/repository/docker/oupfiz5/alt-s6-builder][oupfiz5/alt-s6-builder]]):
#+begin_src sh :tangle no :noweb yes
docker build -t oupfiz5/alt-s6-naviserver:4.99.24 -f ./Dockerfile .
#+end_src
** Configuration
...
Set the http listen port for the container =-p 127.0.0.1:18090:8080=.  In this case the Naviserver-S6 is accessible by URL [[http://localhost:8090][http://localhost:18090]].
#+begin_src sh :tangle no :noweb yes
docker run -itd \
       --restart always \
       --name=alt-s6-naviserver \
...
:END:
For  build and push docker images using  [[https://github.com/oupfiz5/naviserver-s6/blob/master/.github/workflows/on-push.yaml][Github Actions workflow]]. Flow process is [[https://guides.github.com/introduction/flow/][GitHub flow]].
* Troubleshooting
:PROPERTIES:
...
:END:
For debugging and maintenance purposes you may want debug s6 service scripts and\or configuration. In this case container will use scripts and\or configurations from bind directories:
...
#+begin_src sh :shebang ""
  ...
  CONTAINER_NAME="${CONTAINER_NAME:-alt-s6-naviserver}"
  NS_IMAGE_TAG="${NS_IMAGE_TAG:-4.99.24}"
  IMAGE_NAME="${IMAGE_NAME:-alt-s6-naviserver}"
  IMAGE_REPOSITORY="${IMAGE_REPOSITORY:-oupfiz5}"
  IMAGE_TAG="${IMAGE_TAG:-${NS_IMAGE_TAG}}"
  BUILDER_IMAGE_REPOSITORY="${BUILDER_IMAGE_REPOSITORY:-oupfiz5}"
  BUILDER_IMAGE_NAME="${BUILDER_IMAGE_NAME:-alt-s6-builder}"
  BUILDER_IMAGE_TAG="${BUILDER_IMAGE_TAG:-23.05}"
...
:END:
#+begin_src sh :shebang "" :tangle-mode (identity #o755) :tangle no
...
/run/s6/basedir/bin/halt
#+end_src
** rootfs/etc/services.d/naviserver/run :code:tangleno:
:PROPERTIES:
...
#+begin_src sh :results output pp :noweb strip-export :tangle no
  set -e
  IMAGE_NAME="${1:-oupfiz5/alt-s6-builder:22.09}"
